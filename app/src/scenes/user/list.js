...
const NewList = () => {
  ...
  useEffect(() => {
    ...
    setUsersFiltered(
      users
        ...
        .filter((u) => !filter?.job_title || u.job_title === filter?.job_title)
  ...
  console.log("users jobs", users[0].job_title);
      ...
      <div className="pt-6 px-2 md:px-8">
        <div className="flex flex-col-reverse md:flex-row justify-between items-center">
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <FilterJobTitle filter={filter} setFilter={setFilter} />
            <div>
...
const FilterJobTitle = ({ filter, setFilter }) => {
  return (
    <div className="flex">
      <select
        className="w-[180px] bg-[#FFFFFF] text-[14px] text-[#212325] font-normal py-2 px-[14px] rounded-[10px] border-r-[16px] border-[transparent] cursor-pointer"
        value={filter.job_title}
        onChange={(e) => setFilter({ ...filter, job_title: e.target.value })}>
        <option disabled>Job Titles</option>
    ...
    <div
      ...
      <div className="flex flex-col flex-1 justify-between">
        <div className="flex flex-col items-center text-center my-4 space-y-1">
          ...
          <p className="font-regular text-m">{hit.job_title}</p>
        </div>
      </div>
    </div>
  );
